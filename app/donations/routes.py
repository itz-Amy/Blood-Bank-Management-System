...
def _is_donor_eligible(donor_id):
    ...
    active_deferral = (
        Deferral.query
        .filter_by(donorID=donor_id)
        .outerjoin(
            TemporaryDeferral,
            Deferral.deferralID == TemporaryDeferral.deferralID
        )
        .filter(
    ...
    if active_deferral is not None:
        return (
            False,
            "Donor currently has an active deferral and cannot donate."
        )
    ...
    if (
        ...
        return (
            False,
            "Donor has not waited long enough since the previous donation."
        )
...
def _next_blood_unit_id():
    ...
    last_unit = (
        BloodUnit.query
        .order_by(BloodUnit.bloodUnit_id.desc())
        .first()
    )
...
def create_donation():
    ...
    if form.validate_on_submit():
        ...
        eligible, message = _is_donor_eligible(
            form.donor_id.data
        )
        ...
        try:
            ...
            flash(
                "Donation and blood unit recorded successfully.",
                "success"
            )
