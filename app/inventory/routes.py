...
def list_inventory():
    ...
    return render_template('inventory/list.html', items=items, branches=branches)
...
def create_inventory():
    ...
    if form.validate_on_submit():
        ...
        try:
            db.session.add(item)
            db.session.commit()
            flash('Inventory record added.', 'success')
            return redirect(url_for('inventory.list_inventory'))
        except (IntegrityError, OperationalError) as e:
            db.session.rollback()
            flash(f'Could not save: {str(e.orig)}', 'danger')

    return render_template('inventory/form.html', form=form, title='New Inventory Record')


@inventory_bp.route('/inventory/<inventory_id>/delete', methods=['POST'])
@login_required
def delete_inventory(inventory_id):
    item = Inventory.query.get_or_404(inventory_id)
    db.session.delete(item)
    db.session.commit()
    flash('Inventory record deleted.', 'info')
    return redirect(url_for('inventory.list_inventory'))