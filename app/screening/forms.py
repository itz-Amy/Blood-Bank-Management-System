...
class ScreeningForm(FlaskForm):
    ...
    OverallStatus = SelectField('Overall Status', choices=[('Pending', 'Pending'), ('Passed', 'Passed'), ('Failed', 'Failed')], validators=[DataRequired()])
    submit = SubmitField('Save')

    def validate_BloodUnitID(self, field):
        unit = BloodUnit.query.get(field.data)
        if not unit:
            raise ValidationError('Blood unit does not exist. Check the ID and try again.')